...
#ADD '--backend-ignore-cuda' to only use opencl!!!
...
def crack_menu_0():
    ...
    if CEWL_BOOLEAN and FILE_HASH_BOOLEAN or SINGLE_HASH_BOOLEAN and CEWL_BOOLEAN:
        ...
        SINGLE_WORDLIST = os.path.join(WORDLIST_DIRECTORY, 'merged_list/sp00ks_merged_file_uniq.txt')
    hc_cmd1 = ['hashcat', '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'OneRuleToRuleThemAll.rule'), '-w', '3', '-O', '--backend-ignore-cuda']
...
def crack_menu_2():
    ...
    if ALL_MENU:
        ...
        pot_function()
        os.system('clear')
        hc = ['hashcat', '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'OneRuleToRuleThemAll.rule'), '-w', '3', '-O', '--backend-ignore-cuda']
        subprocess.call(hc)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
def crack_menu_3():
    ...
    if CEWL_BOOLEAN and FILE_HASH_BOOLEAN or SINGLE_HASH_BOOLEAN and CEWL_BOOLEAN:
        ...
        SINGLE_WORDLIST = os.path.join(WORDLIST_DIRECTORY, '4GB+/weakpass_2p')
    hc_cmd1 = ['hashcat', '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-w', '3', '-O', '--backend-ignore-cuda']
    subprocess.call(hc_cmd1)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
def crack_menu_5():
    ...
    if ALL_MENU:
        ...
        pot_function()
        os.system('clear')
    hc = ['hashcat', '-a', '6', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '?a?a?a?a', '-w', '3', '-O', '--increment', '--backend-ignore-cuda']
    subprocess.call(hc)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
def crack_menu_6():
    ...
    if ALL_MENU:
        ...
        pot_function()
    os.system('clear')
    hc = ['hashcat', '-a', '7', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), '?a?a?a?a', SINGLE_WORDLIST, '-w', '3', '-O', '--increment', '--backend-ignore-cuda']
    subprocess.call(hc)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
def crack_menu_7():
    ...
    hc1 = ['hashcat', '-a', '7', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), '?d?d?d?d', SINGLE_WORDLIST, '-w', '3', '-O', '--increment', '--backend-ignore-cuda']
    subprocess.call(hc1)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0' or HM_ANSWER == '1' or HM_ANSWER == '2':
    ...
    hc2 = ['hashcat', '-a', '6', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '?d?d?d?d', '-w', '3', '-O', '--increment', '--backend-ignore-cuda']
    subprocess.call(hc2)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
def crack_menu_9():
    ...
    if ALL_MENU:
        ...
        pot_function()
    os.system('clear')
    hc = ['hashcat', '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'd3ad0ne.rule'), '-w', '3', '-O', '--backend-ignore-cuda']
    subprocess.call(hc)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
def crack_menu_11():
    ...
    if ALL_MENU:
        ...
        pot_function()
    os.system('clear')
    hc1 = ['hashcat', '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'leetspeak.rule'), '-w', '3', '-O', '--backend-ignore-cuda']
    subprocess.call(hc1)
    hc2 = ['hashcat', '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'unix-ninja-leetspeak.rule'), '-w', '3', '-O', '--backend-ignore-cuda']
    subprocess.call(hc2)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
#Crack Menu 12 - Rocktastic with OneRuleToRuleThemAll
def crack_menu_12():
    WORDLIST_DIRECTORY = "/mnt/storage-2TB/wordlists" # Needed to reset the wordlist directory.
    global DEFAULT_CEWL_FILE_OUTPUT
    global FILE_HASH_BOOLEAN
...
def crack_menu_13():
    ...
    if ALL_MENU:
        ...
        pot_function()
    os.system('clear')
    hc = ['hashcat', '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'dive.rule'), '-w', '3', '-O', '--backend-ignore-cuda']
    subprocess.call(hc)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
def crack_menu_14():
    ...
    if ALL_MENU:
        ...
        pot_function()
    os.system('clear')
    hc1 = ['hashcat', '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'hob064.rule'), '-w', '3', '-O', '--backend-ignore-cuda']
    subprocess.call(hc1)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0' or HM_ANSWER == '1' or HM_ANSWER == '2':
...
def rsmangler_menu():
    ...
    hc_prepend = ['hashcat', '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'rvrsh3llspecial_prepend.rule'), '-w', '3', '-O', '--backend-ignore-cuda']
    subprocess.call(hc_prepend)
    #Rvrsh3ll Special Append Rule Set
    hc_append = ['hashcat', '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'rvrsh3llspecial_append.rule'), '-w', '3', '-O', '--backend-ignore-cuda']
    subprocess.call(hc_append)
    #Right Side
    hc1 = ['hashcat', '-a', '6', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '?a?a?a?a', '-w', '3', '-O', '--increment', '--backend-ignore-cuda']
    subprocess.call(hc1)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0' or HM_ANSWER == '1' or HM_ANSWER == '2':
...
def cewl_menu_17():
    ...
    if CEWL_BOOLEAN and FILE_HASH_BOOLEAN or SINGLE_HASH_BOOLEAN and CEWL_BOOLEAN:
        ...
        crack_menu()
    pot_function()
    hc = ['hashcat', '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-w', '3', '-O', '--backend-ignore-cuda']
    subprocess.call(hc)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
def cewl_menu_18():
    ...
    subprocess.call(hc1)
    #Left Side
    hc2 = ['hashcat', '-a', '7', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), '?a?a?a?a', SINGLE_WORDLIST, '-w', '3', '-O', '--increment', '--backend-ignore-cuda']
    subprocess.call(hc2)
    #Run against 'dive' Rule
    hc3 = ['hashcat', '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'dive.rule'), '-w', '3', '-O', '--backend-ignore-cuda']
    subprocess.call(hc3)
    #Run Against 'OneRuleToRuleThemAll' Rule
    hc4 = ['hashcat', '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'OneRuleToRuleThemAll.rule'), '-w', '3', '-O', '--backend-ignore-cuda']
    subprocess.call(hc4)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
def prince_menu():
    ...
    try:
        prince_cmd = subprocess.Popen(os.path.join(TOOLS_DIR, 'pp64.bin') + ' --pw-min=' + PW_MIN + ' --pw-max=' + PW_MAX + ' < ' + SINGLE_WORDLIST + ' | ' + 'hashcat ' + HASH_PATH_AND_NAME + ' -m ' + HASH_TYPE + ' --potfile-path=' + os.path.join(L00T_POT_DIR, POT) + ' -r ' + os.path.join(RULES_DIR, 'prince_optimized.rule') + ' -w' + ' 3 ' + '-O ' + '--backend-ignore-cuda', shell=True)
        prince_cmd.wait()
...
def passphrase_menu():
    ...
    if ALL_MENU:
        ...
        pot_function()
    os.system('clear')
    hc = ['hashcat', '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-r', os.path.join(RULES_DIR, 'passphrase-rule1.rule'), '-r', os.path.join(RULES_DIR, 'passphrase-rule2.rule'), '-w', '3', '-O', '--backend-ignore-cuda']
    subprocess.call(hc)
...
def any_menu():
    if ALL_MENU:
        ...
        pot_function()
    os.system('clear')
    hc1 = ['hashcat', '-a', '3', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), '--increment', "?a?a?a?a?a?a?a?a?a?a?a?a", '-w', '3', '-O', '--backend-ignore-cuda']
    subprocess.call(hc1)
    if WIRELESS_BOOLEAN:
        return
    elif HM_ANSWER == '0':
...
def random_rules():
    ...
    if WIRELESS_BOOLEAN:
        ...
        hc = ['hashcat', '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-w', '3', '-O', '-g', '100', '--backend-ignore-cuda']
    elif SELECTION == 'e':
        hc = ['hashcat', '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-w', '3', '-O', '-g', '1000', '--backend-ignore-cuda']
    elif SELECTION == 'f':
        hc = ['hashcat', '-a', '0', '-m', HASH_TYPE, HASH_PATH_AND_NAME, '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), SINGLE_WORDLIST, '-w', '3', '-O', '-g', '10000', '--backend-ignore-cuda']
    subprocess.call(hc)
    if HM_ANSWER == '0':
        hc_cmd2 = ['hashcat', '-m', HASH_TYPE, '-a', '0', '--username', '--session', 'all', '--potfile-path=' + os.path.join(L00T_POT_DIR, POT), '--show', '-o', os.path.join(L00T_POT_DIR, POT + '.format2'), '--outfile-format', '2', HASH_PATH_AND_NAME]
        subprocess.call(hc_cmd2)
...
def single_hash_menu():
    ...
    print" You entered : " + '\n' + " " + single_hash_input
    print""
    #look at adding more interesting stuff here as needed in the future.
    ...
    elif re.search(r"^[a-z0-9A-Z]{32}", single_hash_input): # Likely MD5 or NT HASH Portion
        print(" Likely MD5 or NTHASH Portion" )
